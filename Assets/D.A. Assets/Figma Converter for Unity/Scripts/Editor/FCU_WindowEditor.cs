...
using UnityEditor;
using UnityEngine;
using Group = DA_Assets.Shared.Group;
...
namespace DA_Assets.FCU
{
    public class FCU_WindowEditor : EditorWindow
    {
        ...
        {
            FCU_WindowEditor ccw = GetWindow<FCU_WindowEditor>(LocKey.label_create_script.Localize(FCU_Config.ProductName));
        ...
        private void OnGUI()
        {
            gui.DrawGroup(new Group
            {
                ...
                Body = () =>
                {
                    GUILayout.Label(LocKey.label_you_can_create.Localize(FCU_Config.ProductName), EditorStyles.boldLabel);
                    ...
                    gui.DrawGroup(new Group
                    {
                        ...
                        Body = () =>
                        {
                            GUILayout.Label(LocKey.label_ref_res.Localize(), EditorStyles.boldLabel);
                            EditorGUILayout.Space(gui.NORMAL_SPACE);
                    ...
                    gui.DrawGroup(new Group
                    {
                        ...
                        Body = () =>
                        {
                            GUILayout.Label(LocKey.label_orientation.Localize(), EditorStyles.boldLabel);
                            EditorGUILayout.Space(gui.NORMAL_SPACE);
                    ...
                    EditorGUILayout.Space(gui.NORMAL_SPACE);
                    referenceResolution = EditorGUILayout.Vector2Field(LocKey.label_own_value.Localize(), referenceResolution);
                    EditorGUILayout.Space(gui.NORMAL_SPACE);

                    if (gui.Button(new GUIContent(LocKey.label_create_canvas.Localize())))
                    {
